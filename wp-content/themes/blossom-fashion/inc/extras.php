        ...
        if ($ed_social && $social_links && $echo) { ?>
            <ul class="social-networks">
                ...
                foreach ($social_links as $link) {
                    if ($link['link']) { ?>
                        <li><a href="<?php echo esc_url($link['link']); ?>" target="_blank" rel="nofollow"><i
                                        class="<?php echo esc_attr($link['font']); ?>"></i></a></li>
                        <?php
                    }
                }
                ?>
...
if (!function_exists('blossom_fashion_breadcrumb')) :
    ...
    function blossom_fashion_breadcrumb()
    {
        ...
        if (get_theme_mod('ed_breadcrumb', true) && !is_front_page() && !is_search()) {
            ...
            } elseif (is_tax('blossom_portfolio_categories')  || is_tax('hotel_cat') || is_tax('culinary_cat')) {
                $depth = 2;
            ...
            } elseif (is_tag() && !is_tax('hotel_cat')) {
                    ...
                    <div class="comment-metadata commentmetadata">
                        ...
                        <a href="<?php echo esc_url(htmlspecialchars(get_comment_link($comment->comment_ID))); ?>">
                            <time itemprop="commentTime"
                                  datetime="<?php echo esc_attr(get_gmt_from_date(get_comment_date() . get_comment_time(), 'Y-m-d H:i:s')); ?>"><?php printf(esc_html__('%1$s at %2$s', 'blossom-fashion'), get_comment_date(), get_comment_time()); ?></time>
                        </a>
                    </div>
                </div>
                <div class="reply">
